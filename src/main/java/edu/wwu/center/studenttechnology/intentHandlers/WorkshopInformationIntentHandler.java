...
public class WorkshopInformationIntentHandler extends IntentHandlerBase {
	...
	public WorkshopInformationIntentHandler(WorkshopJsonParser workshopJsonParser) {
		this.workshopJsonParser = workshopJsonParser;
	}

	@Override
	public SpeechletResponse execute(Intent intent) {
		Collection<Workshop> workshopCollection = workshopJsonParser.getWorkshopCollection();
		...
		return SpeechletResponse.newAskResponse(constructOutputSpeech(response), constructReprompt(repromptText));
	}
